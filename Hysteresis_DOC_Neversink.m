%% Title: Dissolved Organic Carbon hysteresis at Neversink River's USGS station #01435000
%If you utilize any part of this code, please cite Husic et al., 2023 (Water Resources Research).

%This code was modified from Husic et al.2023's script for hysetersis anaylsis for stormes at Neversink River's USGS station #01435000

clear all; close all; clc; warning('off'); 
addpath(genpath(pwd)) %this code adds all folders and subfolders within the current director to the path

%% Importing Data
dat = readtable("Neversink River water chemistry.xlsx"); %import Excel folder of data
...
Storm_Cri = 5; %Criterion for storm identification (Q_max must be at least 5 times starting flow, Q_base)
...
close all force %deletes the scatter plot of DOC results with flushing index and hysteresis index

figure('Position',[100 100 1100 800]) % Creates new figure, adjust the figure size as needed
num_storms = length(Storms);

%% edit this section to change ploting styles
rows = 3; %rows and columns of figure
cols = 4; %makes a 3x4 window with 12 plots
single=61; %plots a single storms hysteresis loop and time series. Put the number of the storm here.
t = tiledlayout(rows, cols);

int_storm=1; %initial storm for plotting
fin_storm=12;%final storm to be plotted

storm_range= [4,5,8,10,13,24,28,39,40,45,48,51]; %Can use storm range to plot a specific set of storms that aren't in sequential order
...
%%
for i = int_storm:fin_storm %plots storms between int and fin storm #
    %can use i=storm_range to plot specific storms
    nexttile
    ...
    yyaxis left
    plot(t_plot, q_plot, 'b', 'DisplayName', 'Discharge','LineWidth',2);
    ylabel('Discharge (ft^{3}/s)','FontSize',f_xygrid)
    
    yyaxis right
    plot(t_plot, sc_plot, 'r', 'DisplayName', 'DOC','LineWidth',2);
    ylabel('DOC (mg/L)', 'FontSize',f_xygrid)
    
    hold on; box on; grid minor;
    xlabel('Date', 'FontSize',10)
    title(['Time Series - Storm ' num2str(i)], 'FontSize', f_titlegrid)
    
    legend('Location', 'best', 'FontSize', f_legendgrid); % Add legend with default location
...
for i = single %plots single storm time series
    ...
    title(['Neversink River - Storm ' num2str(i)], 'FontSize', f_titlesingle)
...
%% Next figure with 3x4
figure('Position',[100 100 1200 800])  % Adjust the figure size as needed
...
for i = int_storm:fin_storm 
    ...
    for r = 1:numcolors-1
        plot([norm_q(i,r),norm_q(i,r+1)],[norm_c(i,r),norm_c(i,r+1)],'-','color',colormapcustom(r,:),'LineWidth',3)
        hold on
    end   
    hold on; box on; grid minor;
    ylabel('Normalized DOC Conc.','FontSize',f_xygrid)
    xlabel('Normalized Flow','FontSize',f_xygrid)
    title(['Hysteresis Loop - Storm ' num2str(i)], 'FontSize', f_titlegrid)
end
